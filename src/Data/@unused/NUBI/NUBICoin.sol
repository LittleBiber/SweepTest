...
contract NUBICoin is ERC20 {
    ...
    mapping(uint256 => uint256) public lastTimeMinted;
    mapping(uint256 => address) public lastHolderMinted;
    NFT nftContract;
    ...
    function setMintUnitBySec(uint256 amount) external {
        require(msg.sender == admin, "only admin");
    ...
    function mintWithNFT() external {
        ...
        uint256[] memory tokens = nftContract.walletOfOwner(msg.sender);
        uint256 newCoin = 0;
        uint256 tokenId;
        uint256 newTime = block.timestamp; // 현재 블록의 타임스탬프
        for (uint256 i = 0; i < tokens.length; i++) {
            // 보유한 NFT의 개수만큼 반복
            tokenId = tokens[i]; // NFT배열의 i번째 값이 tokenId
            if (lastHolderMinted[tokenId] != msg.sender) {
                // 대체 뭐지 이거;
                // reset holder
                lastHolderMinted[tokenId] = msg.sender;
                // 홀더가 다르면 그냥 리셋만 하고 넘어간다
            } else {
                // mint for the holder
                // 추가되는 값은 (현재 시간 - 이전에 이 번호의 토큰으로 민팅한 시간) * 1초당 배율
                newCoin += (newTime - lastTimeMinted[tokenId]) * mintUnitBySec;
            }
            // 이 번호의 토큰으로 민팅된 시간을 리셋
            lastTimeMinted[tokenId] = newTime;
            // 그러니까 정리하면
            // 1. 유저가 보유한 토큰 개수만큼 루프
            // 2. i번 토큰으로 민팅한 유저가 다르면 >> 초기화하고 발급양에 포함 X
            //    동일하면 >> (현재 시간 - i번 토큰으로 민팅했던 시간) * 배율을 발급량에 추가
            // 3. i번 토큰으로 민팅된 시간을 초기화
            //
            //! 수도코드
            // 유저가 보유한 NFT 개수만큼 반복
            // 이전 민팅한 유저 addr 과 msg.sender 비교
            // 다르면 skip / 같을때 계속 진행
            // i번 NFT로 이전 민팅시간 call해 배열에 저장
            // 전부 현재시간에서 뺀 다음
            // 전체 합산
            // 합산된 값 * 1초당 배율 * 1000 //
            // 근데 왜 내가 배포한건 lastTimeMinted고 lastHolderMinted 고 아무것도 안나옴
            // 배포가 잘못된건가?
        }
